...
pub trait Query {
    ...
    fn get_headers(&self, client_id: String, user_id: String, request_id: String, timestamp: u64) -> HeaderMap;
}
...
impl Query for TextQuery {
    ...
    fn get_headers(&self, client_id: String, user_id: String, request_id: String, timestamp: u64) -> HeaderMap {
        ...
        request_info.insert("TimeStamp".to_string(), Value::Number(Number::from(timestamp)));
        ...
        header_map.insert("Hound-Request-Info-Length", request_info_json.len().to_string().parse().unwrap());
